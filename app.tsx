...
class TarParser {
  ...
  static async _extractWrapper(file, onProgress) {
    // Scan the plain tar for a nested .tar.gz/.tgz entry, extract it, then process
    let useOPFS = false;
    let opfsRoot;
    try {
    ...
    while (true) {
      ...
      if (fullName.endsWith('.tar.gz') || fullName.endsWith('.tgz')) {
        ...
        if (useOPFS) {
          // Stream to OPFS file (low memory)
          ...
          while (remaining > 0) {
            ...
            onProgress(Math.min(45, 5 + ((entrySize - remaining) / entrySize) * 40), 'Extracting inner archive...');
          }
          ...
          // In-memory fallback: collect inner .tar.gz into chunks
          const chunks = [];
          let remaining = entrySize;
          const buffered = Math.min(remaining, buffer.length - offset);
          if (buffered > 0) {
            chunks.push(buffer.slice(offset, offset + buffered));
            remaining -= buffered;
            offset += buffered;
          }
          while (remaining > 0) {
            ...
            if (done) break;
            const toWrite = value.subarray(0, Math.min(value.length, remaining));
            chunks.push(new Uint8Array(toWrite));
            remaining -= toWrite.length;
            onProgress(Math.min(45, 5 + ((entrySize - remaining) / entrySize) * 40), 'Extracting inner archive...');
          }
          const innerBlob = new File(chunks, fullName.split('/').pop());
          return TarParser._extractGz(innerBlob, onProgress);
        }
      }
  ...
  static async _extractGz(file, onProgress) {
    // Try OPFS (works on HTTPS/localhost), fall back to in-memory (works on file://)
    let useOPFS = false;
    let opfsRoot, tempHandle, tarData;

    try {
      opfsRoot = await navigator.storage.getDirectory();
      useOPFS = true;
    } catch (_) {
      // OPFS not available (e.g. file:// protocol) — decompress to RAM
    }

    const compressedSize = file.size;

    if (useOPFS) {
      // Stream decompress to OPFS temp file (low memory)
      const tempName = '_sysdiagnose_temp_' + Date.now() + '.tar';
      tempHandle = await opfsRoot.getFileHandle(tempName, { create: true });
      ...
      try {
        ...
        while (true) {
          ...
          onProgress(Math.min(48, totalWritten / (compressedSize * 3.5) * 50), 'Decompressing to disk...');
        }
        await writable.close();
      } catch (e) {
      ...
      // In-memory decompression fallback
      const chunks = [];
      let totalWritten = 0;
      const inputStream = file.stream().pipeThrough(new DecompressionStream('gzip'));
      const reader = inputStream.getReader();
      while (true) {
        ...
        chunks.push(value);
        totalWritten += value.length;
        onProgress(Math.min(48, totalWritten / (compressedSize * 3.5) * 50), 'Decompressing to memory...');
      }
      // Concatenate chunks into single ArrayBuffer
      tarData = new Uint8Array(totalWritten);
      let pos = 0;
    ...
    // Set up a unified read interface for both OPFS and in-memory
    const decoder = new TextDecoder();
    let tarSize, tarFile;

    if (useOPFS) {
      tarFile = await tempHandle.getFile();
      tarSize = tarFile.size;
    } else {
      tarSize = tarData.length;
    }
    ...
    const ensureBuffered = async (fileOffset, needed) => {
      ...
      if (useOPFS) {
        ...
        // For in-memory, just point into the tarData buffer directly
        const readSize = Math.max(CHUNK_SIZE, needed);
        const end = Math.min(fileOffset + readSize, tarSize);
        chunkBuf = tarData.subarray(fileOffset, end);
      }
    ...
    tarData = null; // free in-memory buffer
